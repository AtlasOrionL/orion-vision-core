...
export class OrionNetworkDebugger {
    ...
    async startDebugging(): Promise<void> {
        try {
            ...
            if (this.debugPanel) {
                ...
                this.debugPanel.webview.onDidReceiveMessage(
    ...
    private getDebugInterfaceHtml(): string {
        return `
        <!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    private async testRequest(method: string, url: string): Promise<void> {
    ...
    private addRequest(request: INetworkRequest): void {
    ...
    private updateStats(request: INetworkRequest): void {
    ...
    private sendRequestsUpdate(): void {
        this.debugPanel?.webview.postMessage({
    ...
    private showRequestDetails(requestId: string): void {
        const request = this.requests.find(r => r.id === requestId);
        if (!request) {return;}
        
        // Show request details in a new webview or quick pick
        vscode.window.showInformationMessage(
            `Request Details: ${request.method} ${request.url} - ${request.status} (${request.responseTime}ms)`
        );
    }
}
