...
import { OrionAIProvider } from './aiProvider';
import { OrionLogger } from '../utils/logger';

export class OrionCodeCompletionProvider implements vscode.CompletionItemProvider {
    private aiProvider: OrionAIProvider;
    private logger: OrionLogger;

    constructor(aiProvider: OrionAIProvider) {
    ...
    private parseAIResponse(response: string, linePrefix: string, language: string): vscode.CompletionItem[] {
        ...
        try {
            ...
            for (const line of lines) {
                ...
                const completion = new vscode.CompletionItem(
                    trimmedLine,
                    this.getCompletionKind(trimmedLine, language)
                );
    ...
    private getCompletionKind(text: string, language: string): vscode.CompletionItemKind {
    ...
    private enhanceCompletion(completion: vscode.CompletionItem, text: string, language: string): void {
    ...
    private enhanceJavaScriptCompletion(completion: vscode.CompletionItem, text: string): void {
    ...
    private enhancePythonCompletion(completion: vscode.CompletionItem, text: string): void {
    ...
    private enhanceJavaCompletion(completion: vscode.CompletionItem, text: string): void {
        if (text.includes('public ') || text.includes('private ') || text.includes('protected ')) {
            completion.insertText = new vscode.SnippetString(text.replace('()', '($1)$0'));
        }
    }

    async provideCompletions(editor: vscode.TextEditor): Promise<void> {
        if (!this.aiProvider.isAIConnected()) {
            ...
            await vscode.window.withProgress({
                ...
                if (completions && completions.length > 0) {
                    ...
                    if (selected) {
                        ...
                        edit.replace(document.uri, range, selected.item.insertText as string);
