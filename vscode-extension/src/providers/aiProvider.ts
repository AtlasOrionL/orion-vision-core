...
export interface IAIRequest {
    ...
    context?: string;
    language?: string;
    type: 'completion' | 'search' | 'explanation' | 'refactor' | 'debug';
}
...
export class OrionAIProvider {
    ...
    async generateCompletion(request: IAIRequest): Promise<IAIResponse> {
        if (!this.isConnected) {
    ...
    async smartSearch(query: string): Promise<void> {
        if (!this.isConnected) {
    ...
    private getSearchResultsHtml(query: string, response: IAIResponse): string {
        return `
        <!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    async explainCode(code: string, language: string): Promise<IAIResponse> {
    ...
    async refactorCode(code: string, language: string): Promise<IAIResponse> {
        return await this.generateCompletion({
    ...
    async debugCode(code: string, error: string, language: string): Promise<IAIResponse> {
        return await this.generateCompletion({
