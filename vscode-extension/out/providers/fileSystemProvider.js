...
exports.OrionFileSystemProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const configuration_1 = require("../config/configuration");
const logger_1 = require("../utils/logger");
class OrionFileSystemProvider {
    ...
    async _writeFile(filePath, content) {
        if (!this._checkPermission('write'))
            return;
        const operation = {
            id: this._generateId(),
            type: 'write',
    ...
    async _deleteFile(filePath) {
        if (!this._checkPermission('delete'))
            return;
        const operation = {
            id: this._generateId(),
            type: 'delete',
    ...
    async _createSession(name, workingDirectory, permissions) {
        const session = {
            id: this._generateId(),
            name: name,
    ...
    async _copyFile(source, destination) {
        if (!this._checkPermission('read') || !this._checkPermission('write'))
            return;
        const operation = {
            id: this._generateId(),
            type: 'copy',
    ...
    async _moveFile(source, destination) {
        if (!this._checkPermission('write') || !this._checkPermission('delete'))
            return;
        const operation = {
            id: this._generateId(),
            type: 'move',
...
exports.OrionFileSystemProvider = OrionFileSystemProvider;
OrionFileSystemProvider.viewType = 'orionFileSystem';
