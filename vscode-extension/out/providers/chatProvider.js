...
exports.OrionChatProvider = void 0;
const vscode = __importStar(require("vscode"));
const configuration_1 = require("../config/configuration");
const logger_1 = require("../utils/logger");
const node_fetch_1 = __importDefault(require("node-fetch"));
class OrionChatProvider {
    ...
    async _handleSendMessage(content, model) {
    ...
    async _sendToAI(content, model) {
        const baseUrl = `http://${this.config.getServerHost()}:${this.config.getServerPort()}`;
        try {
            const response = await (0, node_fetch_1.default)(`${baseUrl}/api/ai/chat`, {
                method: 'POST',
                headers: {
    ...
    async _createNewSession(title, model) {
        const session = {
            id: this._generateId(),
            title: title,
            messages: [],
...
exports.OrionChatProvider = OrionChatProvider;
OrionChatProvider.viewType = 'orionChat';
